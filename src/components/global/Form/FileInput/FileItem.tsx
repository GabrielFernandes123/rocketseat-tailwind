...
const fileItem = tv({
  slots: {
    container:
      'group flex items-start gap-4 rounded-lg border border-zinc-100 p-4',
    icon: 'rounded-full border-4 border-violet-100 bg-violet-200 p-2 text-violet-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400',
    deleteButton: '',
  },
  ...
  variants: {
    state: {
      ...
      error: {
        ...
        icon: 'border-error-50 bg-error-100 text-error-600 dark:bg-error-500/5 dark:border-error-500 dark:text-error-400',
        deleteButton:
...
export function FileItem({ name, size, state }: FileItemProps) {
  ...
  return (
    <div className={container()}>
      ...
      {state === 'complete' ? (
        <CheckCircle2 className="h-5 w-5 text-violet-600 group-hover:text-violet-700" />
      ) : (
